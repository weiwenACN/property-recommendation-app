      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-[#0F0C2E]">Lifestyle priorities</h2>
            {!isEditing && (
              <button
                onClick={startEditing}
                className="flex items-center gap-1.5 text-[#3C3489] font-medium text-sm hover:underline"
              >
          ...
          {!isEditing ? (
            selectedOptions.length === 0 ? (
              <div className="p-4 rounded-2xl bg-[#F7F6FB] text-center">
                <p className="text-sm text-gray-600">
                  No priorities set yet. Tap <span className="font-medium text-[#0F0C2E]">Add</span> to choose what matters to you.
                </p>
              </div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {selectedOptions.map((option) => {
                  ...
                  return (
                    <div
                      key={option.id}
                      className="flex items-center gap-2 px-3 py-2 rounded-full bg-[#EEEDFE] border border-[#3C3489]/30 text-[#3C3489] text-sm font-medium"
                    >
            ...
            <>
                ...
                {preferenceOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Icon className={`w-7 h-7 mb-2 ${isSelected ? 'text-[#3C3489]' : 'text-[#0F0C2E]'}`} />
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                <button
                  ...
                  className="w-full bg-[#EEEDFE] text-[#3C3489] py-3 rounded-xl hover:bg-[#EEEDFE]/80 transition-colors font-medium"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  className="w-full bg-[#3C3489] text-white py-3 rounded-xl hover:bg-[#2d2766] transition-colors font-medium shadow-lg shadow-[#3C3489]/20"
                >
        ...
        <section>
          <h2 className="text-lg font-semibold text-[#0F0C2E] mb-3">Account</h2>
          <div className="rounded-2xl border border-[#e5e7eb] divide-y divide-[#e5e7eb]">
            <div className="px-4 py-3 flex items-center justify-between">
              <span className="text-sm text-gray-600">Phone number</span>
              <span className="text-sm font-medium text-[#0F0C2E]">Connected</span>
            </div>
            <div className="px-4 py-3 flex items-center justify-between">
              <span className="text-sm text-gray-600">Notifications</span>
              <span className="text-sm font-medium text-[#0F0C2E]">On</span>
            </div>
          </div>
        </section>
      </div>
